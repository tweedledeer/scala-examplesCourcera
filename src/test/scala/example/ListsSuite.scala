 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max element is calculated properly, if it is a last number of sequence"){
    assert(max(List(1,2,12,3,24))===24)
  }

  test("Illegal arg exception thrown if non-int list is passed as an argument"){
    intercept[IllegalArgumentException]{
      Lists.sum(List());
    }
  }

  test("Sum is calculated properly in case if there is only one list element"){
    assert(sum(List(9))===9)
  }

  test("Max is calculated properly in case if there is only one list element"){
    assert(max(List(9))===9)
  }

  test("Max element is calculated taking into account negative values "){
    assert(max(List(1,55,68,-69))===68)
  }
